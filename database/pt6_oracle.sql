...
( deptno number(3) primary key ,
  ...
  part number(3),
  build  varchar2(18))tablespace users;
...
/*
1. student 테이블을 사용하여 학생들의 이름(name), 주민번호(jumin), 성별을 출력하세요.
...
SELECT NAME, JUMIN,
    ...
    FROM STUDENT;
/*
2. student 테이블을 참조하여 학생의 이름(name)과 전화번호(tel), 지역명을 출력하세요.
    (단 지역번호가 02면 서울, 031이면 경기, 051이면 부산, 052이면 울산, 055이면 경남
    나머지는 기타로 표시하세요.)(case문 사용)
*/
SELECT NAME, TEL,
...
/*
3. 학생테이블(STUDENT)과 학과테이블(DEPARTMENT)을 사용하여 학생이름(NAME), 1전공 학과번호(DEPTNO1),
    1전공의 학과이름(DNAME)을 출력하세요.
*/
SELECT DEPTNO1, DNAME
FROM STUDENT S, DEPARTMENT D
WHERE S.DEPTNO1 = D.DEPTNO;

/*
4. 학생테이블(STUDENT)과 교수테이블(PROFESSOR)를 join하여 학생의 이름(NAME)과 지도교수번호(PROFNO),
    지도교수 이름(NAME)을 출력하세요.
*/
...
/*
5. 학생테이블(STUDENT)과 교수테이블(PREFESSOR)를 join하여 학생이름(NAME)과 지도교수 이름(NAME)을 출력
    하세요.
